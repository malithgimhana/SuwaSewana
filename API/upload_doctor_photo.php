...
$doctorUploadsDir = $uploadsDir . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'doctors';
if (!is_dir($doctorUploadsDir) && !mkdir($doctorUploadsDir, 0775, true)) {
    respond(['ok' => false, 'message' => 'Could not create upload directory'], 500);
}

$filename = 'doc_' . bin2hex(random_bytes(10)) . '.' . $allowed[$mime];
$targetPath = $doctorUploadsDir . DIRECTORY_SEPARATOR . $filename;
...
respond([
    'ok' => true,
    'photo_path' => 'uploads/doctors/' . $filename,
]);
